...
public class NMapCalloutCustomOldOverlay extends NMapCalloutOverlay {
    ...
    private static final float CALLOUT_BACKGROUND_HEIGHT =
            CALLOUT_PADDING_Y + CALLOUT_TEXT_SIZE + CALLOUT_PADDING_Y;
    ...
    public NMapCalloutCustomOldOverlay(NMapOverlay itemOverlay, NMapOverlayItem item,
            Rect itemBounds,
        ...
        if (resourceProvider == null) {
            throw new IllegalArgumentException(
                    "NMapCalloutCustomOverlay.ResourceProvider should be provided on creation of "
        ...
        if (mDrawableRightButton != null) {
            if (hasRightAccessory) {
                ...
                mCalloutRightButtonWidth = NMapResourceProvider.toPixelFromDIP(
                        CALLOUT_RIGHT_BUTTON_WIDTH);
                mCalloutRightButtonHeight = NMapResourceProvider.toPixelFromDIP(
                        CALLOUT_RIGHT_BUTTON_HEIGHT);
            }
    ...
    protected Rect getBounds(NMapView mapView) {
        ...
        mTempRect.set((int) mTempRectF.left, (int) mTempRectF.top, (int) mTempRectF.right,
    ...
    protected void drawCallout(Canvas canvas, NMapView mapView, boolean shadow, long when) {
        ...
        if (mDrawableRightButton != null) {
            ...
            if (mRightButtonText != null) {
                ...
                top = mRightButtonRect.top + (mCalloutRightButtonHeight - mTempRect.height()) / 2
                        + mTempRect.height()
                        + mTitleOffsetY;
    ...
    private void adjustTextBounds(NMapView mapView) {
        ...
        if (DEBUG) {
            Log.i(LOG_TAG,
                    "adjustTextBounds: mTempRect.width=" + mTempRect.width() + ", mTempRect.height="
                            + mTempRect.height());
        }
        ...
        float top = mTempPoint.y - (int) (mItemBounds.height() * mOverlayItem.getAnchorYRatio())
                - mItemGapY
                - mTotalHeight;
